...
const MonthProfit = React.memo(({poid, fund_code, type, unit_type, slideFn}) => {
        ...
        () =>
            dateArr?.map((el, index) => {
                ...
                return (
                    <div
                        onClick={() => {
                            if (el.day > currentDay) return
                            getProfitBySelDate(el)
                        }}
                        key={`${el?.id + '' + index}`}
                    >
                        <div style={{...commonStyle.month, ...wrapStyle}}>
                            <span style={{...commonStyle.monthText, ...monthStyle}}>{month}</span>
                            {el?.profit && (
                                <span style={{...commonStyle.monthProfit, ...profitStyle}}>{el?.profit}</span>
                            )}
                        </div>
                    </div>
                )
    ...
    return (
        <div style={styles.container}>
            ...
            {isHasData ? (
                <>
                    {isCalendar && <div style={commonStyle.monthFlex}>{renderCalendar}</div>}
                    {/*{isBarChart && (*/}
                    ...
                    {/*            </div>*/}
                    {/*        </div>*/}
                    {/*        <div style={{marginTop: px(15)}}>{renderBarChart(xAxisData)}</div>*/}
            ...
            <div style={Style.flexBetween}>
                ...
                <div style={styles.selMonth}>
                    {unitList.length > 0 && date != startYear && (
                        <div onClick={subtract}>
                            <img style={{width: px(13)}} src={Prev} alt="" />
                        </div>
                    )}
                    <span style={styles.MMText}>{date}</span>
                    {unitList.length > 0 && date != endYear && (
                        <div onClick={add}>
                            <img style={{width: px(13)}} src={Next} alt="" />
                        </div>
                    )}
                </div>
            </div>
