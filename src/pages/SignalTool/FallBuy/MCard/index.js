...
const MCard = ({extract = false, index_list = [], dateRangeChange}) => {
    ...
    useEffect(() => {
        if (chart.current && chartData) {
            ...
            let option = genChartOption({chartData, gauge_info: chartData?.gauge_info})
            myChart.current.setOption(option)
        }
    }, [chartData])

    const getChartData = (start_date, afterFn) => {
        let params = {tool_id: 2, index_id: active}
    ...
    return (
        <div className={`FallBuyMainCard ${extract ? '' : 'noExtract'}`}>
            {!extract && (
...
const genChartOption = ({gauge_info, chartData}) => {
    ...
    let negativePotions =
        chartData.negative_potions?.map((item) => {
            return [
                {
                    xAxis: item[0],
                    y: 288,
                    ...
                    return {
                        ...
                        detail: {
                            ...
                            rich: {
                                ...
                                c: {
                                    backgroundColor: {
                                        image: 'https://static.licaimofang.com/wp-content/uploads/2022/07/down.png',
                                    },
                                    width: 13,
                                    height: 13,
                                },
