...
const TaurenSignal = () => {
    ...
    const getData = () => {
        http.get('/tool/signal/detail/20220711', {tool_id: 1}).then((res) => {
            if (res.code === '000000') {
                setData(res.result)
        ...
        <div className="TaurenSignal">
            ...
            <div className="cardWrap">
                ...
                <div className="explainTitle">
                    <img className="explainTitleIcon" alt="" src={data.reminder_info?.icon} />
                    <span className="explainTitleText">{data.reminder_info?.title}</span>
                </div>
                <div className="explainDesc" dangerouslySetInnerHTML={{__html: data.reminder_info?.content}}></div>
                <div className="explainTitle">
                    <img className="explainTitleIcon" alt="" src={data.selected_info?.icon} />
                    <span className="explainTitleText">{data.selected_info?.title}</span>
                </div>
                <div className="explainDesc" dangerouslySetInnerHTML={{__html: data.selected_info?.content}}></div>
            </div>
        </div>
    ) : null
}
